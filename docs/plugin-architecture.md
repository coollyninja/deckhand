...
There are two isolation tiers: `in_process` (the default, appropriate only for curated read-only trusted code) and `wasm` (mandatory for third-party or mutation-capable plugins). The `wasm` tier runs a signed Ganglion component under `gang`'s no-ambient-authority capability broker, behind the same `Adapter` protocol and inside the same resilience wrapper. It is disabled by default with `DECKHAND_ALLOW_WASM_PLUGINS=false`. The `wasm` runtime chooses one of two hosts per plugin via the optional `socket` block:
...
- `socket` set — the broker speaks the peer-authenticated Unix-socket host transport to an out-of-process `deckhand-wasm-host` process that embeds `gang-wasm-host`. The host runs under its own UID and a hardened systemd unit, so the component executes behind a **double boundary**: a separate-UID process *and* the WASM no-ambient-authority sandbox. Mutation-capable `wasm` plugins require this out-of-process host. The broker reaches it with the ordinary `WasmHostClient`/`WasmHostAdapter` — the handshake verifies the signed artifact, its locked digest, and the manifest. The host transport never imports the component into the broker: it verifies the locked SHA-256 digest and Ed25519 signature, authenticates both Unix peers, performs a strict handshake, then contributes proxy adapters and status providers behind the normal registry.

`deploy/systemd/deckhand-wasm-host@.service` ships the public unit for the out-of-process host; its hardening (own UID, `IPAddressDeny=any` by default, bounded CPU/memory/tasks/FDs, stripped capabilities and syscalls) is fail-closed with no topology. A private `deckhand-site-<site>` drop-in supplies a service account, broker UID, systemd credentials, and resolved egress CIDRs when a component needs outbound HTTPS; these enforcement bindings never belong in a public plugin or pack. See [ADR 0005](adr/0005-ganglion-wasm-runtime-tier.md). The host transport was re-homed from ADR 0004's now-removed sidecar tier; see [ADR 0004](adr/0004-unix-socket-sidecar-isolation.md) for its historical rationale.
...
- CI runs the same lifecycle contract through bundled, external in-process, and out-of-process wasm-host proxy paths.
